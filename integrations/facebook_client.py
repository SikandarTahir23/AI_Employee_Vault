"""
Facebook Page Client Module for AI Automation Dashboard
Uses Meta Graph API for page posting

Features:
- Token validation and debugging
- Post to page with text, links, and photos
- Get recent posts
...
class FacebookClient:
    ...
    def __init__(self, access_token: Optional[str] = None, page_id: Optional[str] = None):
        ...
        if not self.access_token:
            ...
            raise ValueError(
                ...
                "Set META_ACCESS_TOKEN in your .env file.\n"
        ...
        if not self.page_id:
            ...
            raise ValueError(
                ...
                "Set FB_PAGE_ID in your .env file.\n"
                "Find your Page ID: https://findmyfbid.in/"
            )
    ...
    def validate_token(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        """
        if self._validated and self._page_info:
            return True, self._page_info

        try:
            logger.info("Validating access token...")

            # Try to get page info directly (works without App ID)
            ...
            if 'data' not in token_data:
                ...
                return False, {'error': 'Invalid token response', 'valid': False}

            data = token_data['data']
            is_valid = data.get('valid', False) or data.get('is_valid', False)
            ...
            if is_valid:
                ...
                # Get page info
                self._page_info = self.get_page_info()
                self._validated = True
            else:
                logger.error("✗ Token is invalid")
                error_info = data.get('error', {})
                logger.error(f"  Error: {error_info.get('message', 'Unknown')}")
    ...
    def post_photo(self, message: str, photo_path_or_url: str,
        ...
        try:
            ...
            if photo_path_or_url.startswith('http'):
                ...
                photo_path = Path(photo_path_or_url)
                if not photo_path.exists():
                    return False, f"Photo file not found: {photo_path_or_url}"
    ...
    def get_recent_posts(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for post in posts:
                ...
                logger.debug(f"  Post {formatted_post['id']}: {formatted_post['message'][:50]}...")
...
def get_facebook_page_info() -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        client = FacebookClient()
        return client.get_page_info()
    except Exception as e:
        logger.error(f"Failed to get page info: {e}")
        return None


def get_facebook_recent_posts(limit: int = 5) -> List[Dict[str, Any]]:
    """
    ...
    try:
        client = FacebookClient()
        return client.get_recent_posts(limit)
    except Exception as e:
        logger.error(f"Failed to get posts: {e}")
        return []
