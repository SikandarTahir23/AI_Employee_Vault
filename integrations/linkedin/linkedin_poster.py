...
# Try to use the fixed integrations module first
try:
    from integrations.linkedin_client import post_to_linkedin as integration_post
    from integrations.linkedin_client import connect_linkedin as integration_connect
...
def login_flow():
    ...
    if USE_INTEGRATION:
        # Use the fixed integration module
        print("[INFO] Using integration module for login...")
        success, msg = integration_connect(headless=False, timeout=120)
        print(f"[INFO] Login result: {msg}")
        
        if success:
            print("[INFO] Login successful! Browser will remain open.")
            print("[INFO] Press Enter to close browser and exit...")
            input()
            cleanup_persistent_linkedin()
        else:
            print(f"[WARN] Login failed: {msg}")
    else:
...
def post_to_linkedin(content):
    ...
    if USE_INTEGRATION:
        # Use the fixed integration module - much more reliable
        print("[INFO] Using integration module for posting...")
        print("[INFO] This will open a visible browser window")
        
        success, msg = integration_post(content, headless=False, wait_for_login=True)
        
        if success:
            print(f"[OK] Post submitted: {msg}")
        else:
            print(f"[ERROR] Post failed: {msg}")
